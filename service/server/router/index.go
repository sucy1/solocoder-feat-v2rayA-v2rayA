...
func Run() error {
	...
	)
	{
		...
		noAuth.POST("transparent/remoteSwitch", controller.PostRemoteSwitchProxyMode)
	}
	...
	)
	{
		...
		auth.GET("connection/stats", controller.GetConnectionStats)
		auth.GET("connection/stats/summary", controller.GetConnectionStatsSummary)
		auth.GET("connection/stats/node", controller.GetNodeConnectionStats)
		auth.POST("connection/stats/start", controller.PostStartStatsCollector)
		auth.POST("connection/stats/stop", controller.PostStopStatsCollector)
		auth.GET("connection/stats/config", controller.GetStatsConfig)
		auth.GET("pingLatency", controller.GetPingLatency)
		auth.GET("httpLatency", controller.GetHttpLatency)
		auth.POST("latency/testAll", controller.PostLatencyTestAll)
		auth.GET("latency/status", controller.GetLatencyStatus)
		auth.DELETE("latency/cache", controller.DeleteLatencyCache)
		...
		auth.GET("subscription/notify/status", controller.GetSubscriptionNotifyStatus)
		auth.PUT("subscription/notify/setting", controller.PutSubscriptionNotifySetting)
		auth.POST("subscription/notify/check", controller.PostSubscriptionNotifyCheck)
		auth.POST("subscription/notify/start", controller.PostSubscriptionNotifyStart)
		auth.POST("subscription/notify/stop", controller.PostSubscriptionNotifyStop)
		...
		auth.GET("routing/visual", controller.GetVisualRouting)
		auth.PUT("routing/visual", controller.PutVisualRouting)
		auth.POST("routing/reorder", controller.PostRoutingReorder)
		auth.POST("routing/import", controller.PostRoutingImport)
		auth.GET("routing/json", controller.GetRoutingJSON)
		auth.POST("routing/toJson", controller.PostVisualToJSON)
		auth.POST("routing/toRoutingA", controller.PostVisualToRoutingA)
		auth.GET("routing/colors", controller.GetRoutingColors)
		...
		auth.GET("transparent/status", controller.GetTransparentProxyStatus)
		auth.POST("transparent/switch", controller.PostSwitchProxyMode)
		auth.GET("transparent/modes", controller.GetAvailableProxyModes)
		auth.GET("transparent/apiKey", controller.GetRemoteApiKey)
		auth.POST("transparent/apiKey", controller.PostRemoteApiKey)
		auth.POST("transparent/generateApiKey", controller.PostGenerateRemoteApiKey)
	}
